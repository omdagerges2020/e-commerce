...
const Cards = ({ products, typeGrid }) => {
  return (
    <div className="w-full flex justify-center lg:w-[calc(100% - 300px)]">
      <div
        className={`grid grid-cols-2 xs:grid-cols-1 ${
          typeGrid ? "md:grid-cols-3" : "md:grid-cols-2"
        } ${
          typeGrid ? "lg:grid-cols-3" : "lg:grid-cols-2"
        }  gap-[2em] overscroll-none lg:overflow-y-scroll lg:h-[70vh]`}
        style={{ scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}
      >
        {products &&
          products?.productsList?.data.map((card) => (
            <LazyLoad key={card.id} height={200} offset={100}>
              <Link
                href={`/collections/${card.model}/${card.id}`}
                className={`card w-[300px] xs:w-full ${
                  typeGrid ? "md:w-[230px]" : "md:w-[300px]"
                } lg:w-[230px] bg-white shadow-none rounded-lg`}
              >
