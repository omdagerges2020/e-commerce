  ...
  reducers: {
    addToCart: (state, action) => {
      ...
      if (!checkArr) {
        ...
        //   {
        //     id: action.payload?.productDetails?.data?.data?.id,
        //     quantity: action.payload?.count,
        //     options: {
        //       color: action.payload?.selectedColor,
        //       size: action.payload?.selectedSize,
        //     },
        //     name: action.payload?.productDetails?.data?.data?.product_description?.name,
        //     totalPrice: action.payload?.productDetails?.data?.data?.price,
        //     image: action.payload?.productDetails?.data?.data?.image,
        //   },
        // ];
    
        // تحديث cartProducts في الستات
        // state.cartProducts = {
        //   ...state.cartProducts,
    ...
    deleteProduct: (state, action) => {
      // console.log(action.payload);
      const updatedProducts3 = state.cartProducts.cartData.filter(
        (product) => product.id !== action.payload.id
      );

      state.cartProducts = {
        ...state.cartProducts,
        cartData: updatedProducts3,
      };
      axios
